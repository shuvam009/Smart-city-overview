...
const cityStats = [
  ...
  { label: 'Traffic flow', value: 'High', tone: 'red', detail: '8 active zones' },
  { label: 'Water supply', value: 'Normal', tone: 'green', detail: '98% operational' },
  { label: 'Power status', value: 'Stable', tone: 'blue', detail: '2 planned works' },
]
    ...
    <main className="app-shell">
      <header className="topbar">
        <a className="brand" href="#home" aria-label="Kolkata Smart City home">
          ...
          <span><b>Kolkata</b> Smart City<small>Citizen Command Center</small></span>
        </a>
        <nav className="main-nav" aria-label="Main navigation">
          <a className="active" href="#home">Home</a>
          <a href="#updates">City Updates</a>
          <a href="#contact">Contact</a>
          <button className="notification" aria-label="View notifications">♧<i>3</i></button>
      ...
      <section className="hero" id="home">
        <aside className="sidebar" aria-label="Live city status">
          ...
          <div className="stat-list">{cityStats.map((stat) => <StatCard stat={stat} key={stat.label} />)}</div>
          <button className="report-button"><span>＋</span> Report an issue</button>
      ...
      <section className="portals" aria-label="Access portals">
        <div className="section-intro"><span>ACCESS YOUR PORTAL</span><h2>How would you like to continue?</h2></div>
        <div className="portal-grid">
          <PortalCard icon="⌂" title="Citizen portal" text="Report issues and track their progress." button="Citizen login" variant="citizen" />
          <PortalCard icon="⌁" title="Staff portal" text="Manage assigned work and field updates." button="Staff login" variant="staff" />
          <PortalCard icon="▣" title="Admin portal" text="Monitor operations across the city." button="Admin login" variant="admin" />
        </div>
      </section>

      <section className="updates" id="updates">
        <div><span>CITY UPDATES</span><h2>What’s happening in Kolkata</h2></div>
        <div className="update-list">{notices.map((notice, index) => <div className="update" key={notice}><b>0{index + 1}</b><p>{notice}</p><a href="#contact">View</a></div>)}</div>
      </section>
    </main>
  )
}
